...
//    ans[5][0]=0; ans[5][1]=2; ans[5][2]=1; ans[5][3]=-1; solved[5]=1;
    ...
    int i;
    int j;
    for(i=0; theNumber>0 ; i++){///ex. k=500, i=0 表示個位數, i對應現在的整數
        int now=theNumber%10;
        for(j=0; j<len[k-1]; j++){///ex. 10! = 3628800 ans[10] 是 0088263 (-1...), j對應ans的每一位
            ans[k][j+i] += ans[k-1][j] * now + carry;
            carry=0;
            if(ans[k][j+i]>=10) {carry=ans[k][j+i]/10; ans[k][j+i]%=10; }
            len[k]=j+i+1;
        }
        if(carry>0) {ans[k][j+i]= carry; carry=0; len[k]=j+i+1;}
...
{/// 500!   500! 500! 重覆很多次,只有第一次要真的算, 其他可以查 ans[500]
    ...
    ans[2][0]=2; len[2]=1; solved[2]=1;
    ans[3][0]=6; len[3]=1; solved[3]=1;
    ans[4][0]=4; ans[4][1]=2; len[4]=2; solved[4]=1;
    ans[5][0]=0; ans[5][1]=2; ans[5][2]=1; len[5]=3; solved[5]=1;
